...
@objc(MarkdownQuickPlugin)
public final class MarkdownQuickPlugin: NSObject, VehlaNativeWorkspacePlugin {
    ...
    public var workspaces: [VehlaWorkspaceDescriptor] {
        [
            VehlaWorkspaceDescriptor(
                id: "markdown-quick",
                title: "Markdown Quick",
                subtitle: "QuickGlass Markdown tools",
    ...
    public var quickGlassActions: [VehlaWorkspaceQuickGlassActionDescriptor] {
        MarkdownQuickActionCatalog.all.map(\.descriptor)
    }
    ...
    public func performQuickGlassAction(
        ...
        guard let action = MarkdownQuickActionCatalog.action(id: actionID) else {
            completion.quickGlassActionDidFinish(
                VehlaWorkspaceQuickGlassResult(
                    errorMessage: MarkdownQuickError.unknownAction(actionID).localizedDescription
                )
            )
            return
        }
        ...
        Task { @MainActor in
            do {
                let output = try await MarkdownWorker.shared.perform(
                    action,
    ...
    public func makeViewController(
        ...
        guard workspaceID == "markdown-quick" else {
            throw NSError(
                domain: "com.ibuhs.vehla.markdown-quick",
                code: 1,
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "Unknown Markdown Quick workspace “\(workspaceID)”.",
                ]
            )
        }
        ...
        let controller = NSHostingController(
            rootView: MarkdownQuickWorkspaceView(theme: theme)
        )
